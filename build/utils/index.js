...
function getHtmlTag(tagName, content, attributes, isClosed) {
  ...
  let attributeString = '';
  (0, _map.default)(attributes, (value, attr) => {
    if (Object.prototype.hasOwnProperty.call(attributes, attr) && attributes[attr]) {
      attributeString += " ".concat(attr, "=\"").concat(attributes[attr], "\"");
    }
  });
  const unclosedTag = "<".concat(tagName + attributeString, ">");

  if (isClosed) {
    return "".concat(unclosedTag + content, "</").concat(tagName, ">");
  }
...
function parseRef(capture, state, refNode) {
  const ref = (capture[2] || capture[1]).replace(/\s+/g, ' ').toLowerCase(); // We store information about previously seen defs on
