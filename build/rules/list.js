...
const LIST_BULLET = '(?:[*+-]|\\d+\\.)';
const LIST_ITEM_PREFIX = "( *)(".concat(LIST_BULLET, ") +");
const LIST_ITEM_PREFIX_R = new RegExp("^".concat(LIST_ITEM_PREFIX));
const BLOCK_END_R = /\n{2,}$/;
const LIST_LOOKBEHIND_R = /^$|\n *$/;
const LIST_R = new RegExp("^( *)(".concat(LIST_BULLET, ") [\\s\\S]+?(?:\n{2,}(?! )(?!\\1").concat(LIST_BULLET, " )\\n*|\\s*\n*$)"));
const LIST_ITEM_R = new RegExp("".concat(LIST_ITEM_PREFIX, "[^\\n]*(?:\\n(?!\\1").concat(LIST_BULLET, " )[^\\n]*)*(\n|$)"), 'gm');
...
const list = {
  ...
  parse(capture, parse, state) {
    ...
    const itemContent = items.map((item, i) => {
      ...
      const space = LIST_ITEM_PREFIX_R.exec(item)[0].length; // And then we construct a regex to "unindent" the subsequent
      // lines of the items by that amount:

      const spaceRegex = new RegExp("^ {1,".concat(space, "}"), 'gm'); // Before processing the item, we need a couple things
  ...
  react(node, output, state) {
    if (node.ordered) {
      return _react.default.createElement("ol", {
        start: node.start,
        key: state.key
      }, (0, _map.default)(node.items, (item, i) => _react.default.createElement("li", {
        key: i
      }, output(item, state))));
    }
  ...
  html(node, output, state) {
    const listItems = node.items.map(item => (0, _utils.getHtmlTag)('li', output(item, state))).join('');
